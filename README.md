...
Daily aggregator of New Hampshire-located taildragger classified
...
[Nevada](https://github.com/taildraggers/nevada) repos' approach, with the
state filter swapped to "NH".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, NH` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in New Hampshire"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/new-hampshire/`
...
```html
<iframe
  src="https://taildraggers.github.io/new-hampshire/"
  title="Taildragger Ads in New Hampshire"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_NEW_HAMPSHIRE_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
