"""Aggregates New Hampshire-located listings from every taildraggers.com
...
ones whose location ends in ", NH".
...
Minnesota/Mississippi/Missouri/Montana/Nebraska/Nevada repos' approach, with
the state filter swapped to "NH".
"""
...
PAGE_TITLE = "Taildragger Ads in New Hampshire"
STATE_ABBR = "NH"
...
_NEW_HAMPSHIRE_LOCATION_RE = re.compile(r",\s*NH\s*$", re.IGNORECASE)


def _is_new_hampshire(location: str) -> bool:
    return bool(_NEW_HAMPSHIRE_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        new_hampshire_rows = [row for row in rows if _is_new_hampshire(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(new_hampshire_rows)} in New Hampshire")
        all_listings.extend(new_hampshire_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No New Hampshire listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique New Hampshire listings: {len(listings)}")
